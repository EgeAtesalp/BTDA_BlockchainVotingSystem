...
describe("🔬 SCALING ANALYSIS EXPERIMENTS", function () {
    let results = [];
    ...
    describe("📈 EXPERIMENT 1: Voter Count Scaling (Fixed Districts)", function () {
        ...
        it("Should analyze performance across different voter counts", async function () {
            ...
            console.log("🔬 EXPERIMENT 1: VOTER COUNT SCALING ANALYSIS");
            console.log("=".repeat(80));
            console.log("📊 Testing different voter counts with fixed district count");
            console.log("🎯 Objective: Analyze how system scales with voter population");
            console.log("=".repeat(80));
    ...
    describe("🏛️ EXPERIMENT 2: District Count Scaling (Fixed Total Voters)", function () {
        ...
        it("Should analyze performance across different district counts", async function () {
            ...
            console.log("\n🔬 EXPERIMENT 2: DISTRICT COUNT SCALING ANALYSIS");
            console.log("=".repeat(80));
            console.log("📊 Testing different district counts with fixed total voter count");
            console.log("🎯 Objective: Analyze how system scales with district parallelization");
            console.log("=".repeat(80));
            
            // Test configurations: Fixed 100,000 total voters, varying district counts
            const totalVoters = 10000;
            const testConfigurations = [
                { districts: 1, votersPerDistrict: 10000, name: "Single District" },
                { districts: 2, votersPerDistrict: 5000, name: "Dual Districts" },
                { districts: 5, votersPerDistrict: 2000, name: "Penta Districts" },
                { districts: 10, votersPerDistrict: 1000, name: "Deca Districts" },
                { districts: 20, votersPerDistrict: 500, name: "Multi Districts" },
                { districts: 50, votersPerDistrict: 200, name: "Ultra Districts" },
            ];
    ...
    after(function() {
        ...
        saveResults(results, 'complete_scaling_analysis_par');
...
async function runScalingExperiment(config, experimentType) {
    ...
    try {
        ...
        // Phase 5: Voter Registration
        console.log("Phase 5: 📝 Voter Registration");
        ...
        const registrationPromises = districts.map(async (district) => {
            const batchSize = 50; // Use same as your experiment
            let districtGas = 0;
        ...
        const gasResults = await Promise.all(registrationPromises);
        totalRegistrationGas = gasResults.reduce((sum, gas) => sum + gas, 0);
        ...
        // Phase 6: Voting
        console.log("Phase 6: 🗳️ Parallel Voting");
        ...
        const votingPromises = districts.map(async (district) => {
            ...
            const maxConcurrent = Math.min(50, district.voters.length);
            
            for (let i = 0; i < district.voters.length; i += maxConcurrent) {
                const chunk = district.voters.slice(i, i + maxConcurrent);
                
                const chunkPromises = chunk.map(async (voter) => {
                    try {
                        ...
                        return { success: true, gas: Number(receipt.gasUsed) };
                    } catch (error) {
                        return { success: false, gas: 0, error: error.message };
                    }
                });
            ...
            return { successful: districtSuccessful, failed: districtFailed, gas: districtGas };
        });
        
        const votingResults = await Promise.all(votingPromises);
        
        votingResults.forEach(result => {
            successfulVotes += result.successful;
            failedVotes += result.failed;
            totalVotingGas += result.gas;
        });
...
function generateCSVData(results) {
    ...
    const csvPath = path.join('./results', 'scaling_analysis_par.csv');
...
function generateSummaryReport(results) {
    ...
    const reportPath = path.join('./results', 'scaling_summary_par_report.json');
